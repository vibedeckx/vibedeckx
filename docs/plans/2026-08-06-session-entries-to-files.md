...
>
> **本文是落地计划。** 它依据的通用决策模型（退路矩阵、代号机制、三道门）见
...
| 2 `native_session_id` | ✅ **完成**（2026-08-08，`80b5358`） | 定位已升级：不只是调试便利，是**灾后恢复指针**。列 + 两个 provider 冒泡 + manager 落库，纯 expand |
| 3 entries 出库 | ⏳ **决定悬置**（2026-08-08） | 方向不变（分类正确），日期不定。悬置是安全的，前提是接口纪律，见下 |

### 立场修正（2026-08-08，替代此前的「等触发条件」与「R1 提前」两版）

两个此前论证的修正，都有实测支撑：

1. **「未来消费者在变多，所以要赶紧切」不成立。** FTS5 / intent-brief 蒸馏 /
   multi-level commander / workflow review loop 四个排队需求全部是**按 session 整段读**
   （走 `storage.agentSessions.getEntries()` 接口），对介质中立；FTS5 索引的输入也是
   「解析 JSON 抽取文本」的派生投影，不绑定介质。全部消费调用点实测：
   `agent-session-manager.ts` 9 处（全走接口）+ search-cache 一处 `EXISTS` 直接 SQL。
   **切换成本被钉死在 repository 层，不随消费者数量增长**——只要守住接口纪律：

   > **新代码访问 entries 只准走 `getEntries` / `upsertEntry`；禁止新增直接对
   > `agent_session_entries` 的 SQL（含 FTS5 `content=` 绑定）。**

2. **「下次重建 `agent_sessions` 会付一次税」不成立。** 实测 `ccc4651`（2026-08-05）
   那次头表重建：只拷头表 1,640 行，entries 作为子表一行不动（`foreign_keys=OFF`
   挡住了 `DROP TABLE` 的隐式 CASCADE）。头表重建频繁 ≠ entries 收税。
   真正的风险是那个 CASCADE **静默删光** 29 万行的失败模式——由守卫测试（①）锁死，
   不需要文件化来解。

因此文件化**方向保留、日期悬置**，真正逼拍板的时刻只剩三种：
要对外承诺「删了就是删了」（隐私，DB 形态下 `DELETE` 后数据仍在 freelist 页里可被
`strings` 捞出）、FTS5 设计评审时有人想绑表、或库真出一次事故。

### 未雨绸缪包（防灾底线，2026-08-08 决定，✅ 当日全部实施完毕）

> 实施提交：`9e8d101`（① 守卫测试）、`80b5358`（④ native_session_id）、
> `765549b`（②③ 头备份轮转 + 启动自愈，`storage/head-backup.ts`）。
> ① 经 sabotage 验证（人为把迁移期 `foreign_keys` 反转为 ON，测试确实变红——
> 注意 strip 场景的过渡表必须保留 PRIMARY KEY，否则子表约束解析不到唯一索引，
> 级联永远不会触发，测试空转变绿）。
> ② 的实现要点：备份在**打开库之前**做（旧的已知良好二进制留下的状态，本次迁移
> 写坏也污染不到当天的备份）；DDL 从 `sqlite_master` 原样重放（未来加列自动携带）；
> 复制期间必须显式 `foreign_keys = OFF`——better-sqlite3 默认开着，而重建过的
> 父表（agent_sessions）在 `sqlite_master` 里排在引用它的子表之后。
>
> 补遗（2026-08-08 当日，`dc13064` / `4a81e26`）：
> - **§6.1 规则 4（接口纪律）现在有测试强制**：`entries-interface-discipline.test.ts`
>   扫描 `src/`，表名出现在 `src/storage/` 之外的非测试代码即红（金丝雀验证过）。
> - **备份加 6h 定时器**：worker 守护进程数周不重启，仅启动时备份会过期；
>   `maintainHeadBackup` 对当天已有文件是空操作，tick 近乎免费，`close()` 清除、
>   `unref()` 不阻进程退出。

**恢复能力不挂在任何触发条件上**——事故发生在用户机器上时你无法事后补救，只能事前铺好。
现状审计（2026-08-08）：存储层**没有任何**备份 / 完整性检查 / 损坏处理逻辑；
`data.sqlite` 损坏 = executors / rules / settings 全丢，会话历史与 CLI 侧 JSONL 对不上号。

四件事，**与「entries 留库还是出库」无关**，两种未来下都原样成立：

| # | 内容 | 作用 |
|---|---|---|
| ① | **FK 守卫测试**：建库→塞 entries→跑头表重建→断言行数不变且 `REFERENCES` 未被改写 | 锁死最可能的事故类别（自家迁移写错 → CASCADE 静默删光） |
| ④ | **`native_session_id`**（即原第 2 步） | 会话正文唯一幸存副本（CLI 自己的 JSONL）的指针；库坏后可部分恢复对话 |
| ② | **关系型头自动备份轮转**：除 entries 外全部表（实测仅 3.8 MB）导出到 `backups/head-<date>.sqlite`，留 7 份 | 「session 丢了、其它都能恢复」的直接实现。整库 444 MB 轮转不起，选择性导出绕开 |
| ③ | **启动自愈**：打开失败 / `SQLITE_CORRUPT` → 坏文件挪到 `data.sqlite.corrupt-<ts>`（保留不销毁）→ 从最新头备份恢复 → 大声记日志继续启动 | 符合存储经验文 §2 ③（用户机器不能跑 runbook，只能自愈） |

残余风险（承认的）：会话正文本身仍无真正备份（444 MB 轮转不现实），兜底是 CLI 的 JSONL
（经 ④ 对号，缺 vibedeckx 叠加语义）。彻底解由将来的文件化提供（坏一个文件丢一场对话），
但那是方向问题；本包是现在就该有的地板。

实施注意：`quick_check` 在 444 MB 库上实测 **25 秒**——健康检查只能按需触发
（打开报错时），不能每次启动全量跑；备份失败不得阻塞启动。

**一个独立于本计划的小动作**（§7.0.3）：在 worker 握手的 `status` 帧加 `dataDirBytes`，
在已有的 `/api/admin/worker-version-stats` 上多聚合一列。半天的活，把"磁盘要不要管"
从猜变成一条分布曲线。

### 相关提交

文档线：`ce178da`（经验文）、`573ee78..8bfbec5`（本计划 + 通用模型）、
`dcaadc4`（未雨绸缪包决定 + 触发条件修订）。
代码线（2026-08-08，未雨绸缪包）：`9e8d101` / `80b5358` / `e216a6f` / `765549b`。
**entries 本体仍零改动**——`search_1.sqlite` / `entries.jsonl` / `entries_location`
在代码库中均无实现，第 1/3 步维持搁置/悬置。
...
> ✅ **已完成**（2026-08-08）。下列规则已落进存储经验文 §6.1，Q4 已落进 §6，
> 并顺带修正了原 §6 的一处错误——Q1 单独就判"文件"，与 §10.1 的退路矩阵冲突
> （hub 侧的源真相该留库）。**判据以经验文 §6 为准**，本节保留推导过程。
...
不要为了「架构干净」做。等下面任一条出现，**按强度排序**：

1. **要认真对待"删掉某一场会话的内容"**（隐私诉求，非空间）。
   "我在那个 session 里粘了生产库密码" 是立刻、单条、确定性的诉求。
   在库里 `DELETE` 之后数据仍在 freelist 的页里，`strings data.sqlite` 还能捞出来——
   **这个功能在 DB 形态下是半假的**，真正意义上的删除需要 entries 是文件（`rm` + 覆盖语义清晰）。
   这是目前**最强**的一条触发条件。
2. worker 库体积或损坏出过一次事故（未雨绸缪包已把爆炸半径的**关系型头**部分兜住，
   见 §0.0；事故若真发生，丢的主要是会话正文——那正是文件化能改变的部分）。
3. FTS5 联邦搜索需要直接扫记录（注意：索引输入是抽取文本的派生投影，本身不绑介质；
   构成触发的是评审中出现 `content=agent_session_entries` 这类绑表设计）。
4. Remote Servers 页面要显示各 worker 的磁盘占用并提供清理（见 §7.0.1）——
   **但这条本身的产品价值存疑，见 §7.0.3。别为了触发第 3 步去做它。**

> ~~原第 2 条「下一次 `agent_sessions` 需要重建表」~~ **已撤销**（2026-08-08）：
> 实测 `ccc4651` 那次头表重建只拷 1,640 行头表，entries 作为子表一行不动，
> 不存在"284k 行的税"。该条的真实内核（CASCADE 静默删光的风险）由未雨绸缪包的
> 守卫测试锁死，不构成文件化的触发条件。
...
### 7.0.3 产品判断：这个功能本身现在不值得做（2026-08-08）

§7.0.1 论证的是「**若**要做，就必须先出库」，不是「应该做」。产品上的判断相反：

**磁盘不是真需求。** 本机是用量上限（开发者自己的机器，近乎不间断跑 agent），
增长曲线约 120 MB/月且在加速：
...
worker 跑在开发者笔记本上（512 GB 起步），一年 1.5 GB 是噪音。对标也一致：
Codex 8.5 个月 1393 个 rollout、Claude Code 2082 个 jsonl，**两家都没有清理功能，
也没有磁盘占用显示**。一个要靠「用了一年可能有点大」来论证的功能，就是还没到时候。

**里面藏着的两个真需求都不是「清理」**：

- **删掉某一场会话的内容** —— 隐私诉求，立刻/单条/确定性。对应的 UI 是会话上的删除，
  不是 Remote Servers 页面的批量清理。已提升为 §7.0 的第 1 条触发条件。
- **透明度**（这东西在我机器上存了什么）—— 完整答案是设置页里的一行字
  （路径 + 数字 + 打开目录），不是一套带策略的功能。

**推荐动作：先测量，别先建功能。** 在 worker 握手的 `status` 帧里加一个 `dataDirBytes`，
在已有的 `/api/admin/worker-version-stats` 聚合端点上多聚合一列。没有 UI、没有清理、
没有策略讨论；加法变更（老 worker 不报 → 显示「未知」），按隧道契约登记 capability。
半天的活，且**独立于本计划**。

买到的是把一个靠直觉猜的产品问题变成一条分布曲线：若三个月后 p95 是 200 MB，
这个功能永远不必做；若有人已经 8 GB，那要做的也不是清理按钮，而是先查清他为什么长那么快
（多半是某类工具输出把 entries 撑大了——那是 bug，不是容量问题）。
...
不是「用户控制部署 ⇒ 用文件」——那条因果不成立。数据库完全能在版本倾斜下活着，
只要坚持加法改动；Codex 的 `state_5.sqlite` 装在无数台用户机器上，42 条迁移
绝大多数是 `ADD COLUMN`，就是证明。

真正的问题是：**当一个加法改不动的变更出现时（改主键、改类型、重划结构），
你手上还剩几张牌。**

| | ① 你控制部署（hub） | ① 用户控制部署（worker / solo） |
|---|---|---|
| ② **派生数据** | migrate ✓ | **discard ✓**（换代，数据丢了能重建） |
| ② **源真相** | migrate ✓ | **两张牌都没有** ← 唯一被困的格子 |

- `migrate`：eager 全量转换，改完所有实例都是新形状。只有你控制部署时才成立。
- `discard`：改不动就换代，从零重建。只有数据可重建时才成立。
- 被困的那一格：迁移要在你不运维的机器上做不可回滚的重建表（写错 = 用户对话历史没了，
  §4.2）；丢弃则直接丢用户数据。

**文件是第三条路，有效的原因很直白：它没有 schema，所以既不需要迁移也不需要换代。**
格式演进由读端容忍消化（entries 现有那批 optional 字段），一条记录读错能修能回滚，
不会出现「整张表毁了」。

反向验证：Codex rollout（worker+真相）→ 文件；Codex `state_5`（worker+派生）→ 代号库；
`project_chat_messages`（hub+真相）→ 留库；entries（worker+真相）→ 文件。
**Codex 的分层与这个 2×2 完全吻合**——同一约束下的同一个解。

### 另外两道门

**③ 文件在技术上是否可行**（与退路无关，独立判断）：

- **③a 追加写**，没有真正的就地更新需求。entries 有 `upsertEntry`，
  但查下来只是幂等重写，append-only + last-wins 回放天然等价。
- **③b 不需要跨记录查询。** entries 能出去，是因为全仓库只有一处
  （`search-cache.ts:615` 的 `EXISTS`）。有关系型邻居的（如
  `project_chat_work_items.user_message_id` 引用消息）不适合。
  若被困格子里的数据**又需要**跨记录查询，正确形态是**文件 + 一个可换代的索引库**
  （FTS5 联邦搜索就是这个形状），而不是退回单库。

**④ 现在值不值得动**：跟着**每条消息 / 每次输出**增长的才有紧迫性。
④ 是增长率不是当前大小——决策发生在建表之前，那时你只知道它跟着什么增长。
不满足 ④ 只是没必要动，不是放错了地方（`turn_snapshots` 即此类）。

**一句话**：①② 决定去哪一层，③ 决定文件形态是否可行，④ 决定什么时候动。

> **补充（诚实起见）**：被困格子里也可以赌「永远只做加法改动」，用一个纯加法的库撑下去。
> 但那是在赌未来几年不会出现需要重建表的变更，而且照样拿不上 `VACUUM`、
> 爆炸半径覆盖全库、吃 native ABI（`better-sqlite3`）。
> **文件的价值是取消这个赌，不是「文件天生更好」。**

三条去处正好对应 §1 的三层：

| 情况 | 去哪 |
|---|---|
| 被困格子（用户控制部署 + 源真相），且满足 ③ | **文件**（源真相层） |
| 派生数据 | **代号库**（可丢弃层） |
| hub 侧 / 有关系型邻居 | **留库**（关系型头层，hub 随 Postgres 走正常迁移） |
