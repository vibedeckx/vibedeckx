...
| 3 entries 出库 | ⏳ **等触发条件** | 见下；触发条件已按强度重排（§7.0） |
...
**第 3 步在等触发条件，按强度排序（完整列表见 §7.0）**：

1. 要认真做「删掉某一场会话的内容」（隐私）——**最强**。DB 形态下这个功能是半假的：
   `DELETE` 后数据仍在 freelist 页里。
2. 下一次 `agent_sessions` 需要重建表——**大概率最先到来**，因为本项目 schema 一直在动。
   那一刻要处理 284k 行的 FK 子表，税是切实的。
3. 库出事故 / FTS5 要扫记录。
4. Remote Servers 磁盘占用 + 清理——**该功能本身产品价值存疑，见 §7.0.3；
   不要为了触发第 3 步去做它。**
...
1. **要认真对待"删掉某一场会话的内容"**（隐私诉求，非空间）。
   ...
   **这个功能在 DB 形态下是半假的**，真正意义上的删除需要 entries 是文件（`rm` + 覆盖语义清晰）。
   这是目前**最强**的一条触发条件。
2. **下一次 `agent_sessions` 需要重建表**（改主键 / 改类型 / 重划结构）。
   届时必须处理 `agent_session_entries` 那个 FK 子表（§2 的 `RENAME` 陷阱）+ 284k 行的拷贝。
   这个项目的 schema 一直在动（workspace checkout 外键、executor 作用域、几个 saga 表），
   所以这一刻大概率先于其他条件到来。**那时你会切实付一次税，也才有真实依据判断值不值。**
3. worker 库体积或损坏出过一次事故。
4. FTS5 联邦搜索需要直接扫记录。
5. Remote Servers 页面要显示各 worker 的磁盘占用并提供清理（见 §7.0.1）——
