...
tick:
  ...
  loop:
    ...
    逐个删除：**每条删除前** setImmediate 让出事件循环（better-sqlite3 是同步的，
             批间让出会把 20 条删除背靠背执行 → 实测冻结 916ms，见下方压测结果），
             然后重新验证谓词（见 §1.5）
...
删除本身可能超过预算——预算封的是"批的数量"，不是硬上限。

✅ **压测结果（2026-08-09，本条待办已完成）**：单条父行 DELETE + CASCADE，
每 session 2KB/entry：

| entries/session | 每 session 耗时 | 一个 30s tick 能清 |
|---|---|---|
| 1,000 | 16.5 ms | ~1,800 |
| 5,000 | 227 ms | ~132 |

单条删除远没到 30s，**§1.3 不需要渐进删除协议**，v1 单条父行 DELETE 的结论成立。

⚠️ **但压测同时暴露一个真问题：批间让出不够，必须每条让出**（2026-08-09 实测修正）。
`better-sqlite3` 是**同步**驱动——`await storage...deleteIfExpired()` 只是 resolve 一个
已完成的 promise，CASCADE 实际在主线程上跑。原实现只在**批边界** `setImmediate`，
于是 20 条删除背靠背执行，实测（3000 entries/session）事件循环**冻结 916 ms**：
这段时间 WebSocket 不收帧、HTTP 不响应、agent stdout 不解析。5000 entries 时约 4.5 s。
改成**每条删除前** `setImmediate` 后，最大停顿 916 ms → **36 ms**（即单条语句本身的
代价，与 app 里任何一次普通查询同量级）。
批大小 20 因此只是「重读配置 / 推进游标 / 检查预算」的粒度，**不再是让出粒度**。
但上面「一两个 tick 内清完」的说法**只对轻量 session 成立**：5000-entry 的重型
session 每 tick 只能清 132 个，两千个这样的 session 需要 ~15 个 tick——按 6h 一个
tick 就是**约 4 天**。首次启用的排空速度因此不取决于批大小，而取决于**触发间隔**。
曾考虑「`budgetExhausted` 时短延迟自催一次」把首启排空压到分钟级
（≈22 分钟）。❌ **2026-08-09 决定不做**，理由是收益经不起推敲：

- **稳态永不触发**：稳态 sweep 一次空 SELECT 就结束，`budgetExhausted` 恒为 false。
  这个机制只为「首次启用 + 大量重型 session」这一个一次性事件存在。
- **快排空并不提前还盘**：§4.1 已定「只封顶不缩盘」——删除的页进 freelist、
  不自动 VACUUM、界面也不显示磁盘数字。4 天 vs 22 分钟的**唯一**用户可见差别，
  是 90 天前的未加星会话从侧边栏/搜索里早几天消失。没人在等这件事。
- **代价却是真的**：自催把「30s 干活 / 60s 空闲」变成持续占空比 33% 的同步
  SQLite 写入负载。每条让出只压住了**单次**停顿（重型 session 单条 CASCADE 仍
  ~227 ms），「单次停顿可接受」**不等于**「持续负载无影响」——后者从未实测。
  真要做，前置条件是先在 catch-up 负载下压一次端到端 HTTP / WebSocket 延迟。

6h 节奏本身是收敛的、可中断不丢进度、自我纠正的；首启多花几天清理一批早已死掉的
会话，是完全可接受的一次性瞬态。**别再重新讨论这条，除非出现「必须尽快还盘」的
新需求——那属于 §4.1 的 `maintenance compact`，不是调度问题。**
