...
若要让首启在分钟级排空，做法是「`budgetExhausted` 时短延迟自催一次」，而不是加大
批或放宽预算（那会把 I/O 压力压到正在服务的用户身上）。**此项未实施，留作决策点。**

**自催的前置条件（不要跳过）**：每条让出把**单次**停顿压到了一条语句的代价，但
重型 session 的单条 CASCADE 仍要 ~227 ms，而自催会把「30s 干活 / 60s 空闲」变成
持续占空比 33% 的 SQLite 写入负载。「让出解决了单次停顿」**不等于**「持续负载
无影响」——前者已实测，后者没有。实施自催之前必须先在 catch-up 负载下压一次
端到端的 HTTP / WebSocket 延迟；不达标就维持 6h 节奏（首启多花几天是可接受的
一次性代价），或者把删除挪出主线程。
