...
（典型：bug fix）。机器判断不了急不急——先给结论再提问：

1. **明确告知「worker 端有更新」**：这批改动落在 worker 侧，只部署 server 到不了
   用户手上，必须打 tag 发 npm、用户升级 worker 才生效。这句话要说出来，不要只
   甩一个文件清单让用户自己推断。
2. 列出 gray 文件清单和各自改了什么（读 diff 概括），用用户能感知的影响描述
   （"修了 X 场景下 Y 不生效"），而不是函数名。
3. 然后**问用户**：这些修复对用户急不急？

- 急 → 建议：部署 server + 打 tag + 催升级。
- 不急 → 建议：部署 server，修复攒着随下次 tag 一起发（明说：在那之前用户仍会
  碰到这些问题）。
